    ...
    RemoteEventListener		// So we can receive chat notifications
{
      ...
      RemoteException
  {
    if (rev instanceof ChatNotification) {
      ...
      String uuidText = msgText.substring(0, msgText.indexOf('$'));
      String payload = msgText.substring(msgText.indexOf('$') + 1);

      UUID incomingUUID = UUID.fromString(uuidText);
      if(!msgSet.remove(incomingUUID)) {
  ...
  public void sendToChat (String text) {
    if (myServer != null) {
      ...
      msgSet.add(uuid);
      try {
        String taggedText = uuid.toString() + "$" + text;
	      myServer.say (taggedText);
      } catch (RemoteException rex) {
        msgSet.remove(uuid);
