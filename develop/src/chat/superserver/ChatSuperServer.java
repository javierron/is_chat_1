...
/**
 * This class implements the ChatSuperServer application.
 */
public class ChatSuperServer
  extends
    ...
    RemoteEventListener		// So we can receive chat notifications
{
  ...
  protected HashMap<String,ChatServerInterface> myServers = null;
  ...
  /**
   * Creates a new ChatSuperServer instance.
   */
  public ChatSuperServer ()
    throws
      ...
      RemoteException
  {
    ...
    myServers = new HashMap<String, ChatServerInterface>();
  }
      ...
      RemoteException
  {
    ...
    if (rev instanceof ChatNotification) {
        ...
        Set<java.util.Map.Entry<String, ChatServerInterface>> entries = myServers.entrySet();

        String msgText = chat.getText();
        for (java.util.Map.Entry<String, ChatServerInterface> server : entries) {
               server.getValue().say(msgText);
          }
      }
   ...
   */
  public void userDisconnect (String serverNamePattern) {

    ChatServerInterface selectedServer = null;
    String selectedName = "";
    Set<java.util.Map.Entry<String, ChatServerInterface>> entries = myServers.entrySet();

    for (java.util.Map.Entry<String, ChatServerInterface> server : entries) {
        if (server.getKey().startsWith(serverNamePattern)) {
          selectedServer = server.getValue();
          selectedName = server.getKey();
          break;		// out of for each name string
        }
      }
    if (selectedServer != null) {
      disconnect (selectedServer);
      myServers.remove(selectedName);
    }
    else {
  ...
  public void connectToChat (String serverNamePattern)
  {
	...
	if (nextServer != null) {
	  if (myServer != null)
	  myServers.put(selectedName, nextServer);
	}
      }
  ...
  public void sendToChat (String text) {
    if (myServer != null) {
      try {
         Set<java.util.Map.Entry<String, ChatServerInterface>> entries = myServers.entrySet();
          for (java.util.Map.Entry<String, ChatServerInterface> chatServer : entries) {
              chatServer.getValue().say(text);
          }
      } catch (RemoteException rex) {
  ...
  public void readLoop () {
	...
	if ("quit".startsWith (verb)) {
	  ...
	  userDisconnect (stringJoin (argv, 1, " "));
	}
	  ...
	  System.out.println ("[" + verb + ": unknown command]");
	    ...
	    System.out.println("[unable to perform command]");
      }
    ...
    for (java.util.Map.Entry<String, ChatServerInterface> chatServer : entries) {
        disconnect (chatServer.getValue());
    }
      ...
      RemoteException
  {
    ...
    ChatSuperServer cc = new ChatSuperServer ();
    cc.readLoop ();
    System.exit(0);
  }
}
